...
export class CloudinaryImageRequestHelper {
    ...
    static getImageUrl(publicId: string, format: CloudinaryImageFormat, density: CloudinaryImageDensity, width?: number, height?: number): string {
        const dprParameter = `dpr_${density.toFixed(1)}`
        const widthParameter = width ? `,h_${width}` : '';
        const heightParameter = height ? `,h_${height}` : '';
        return `${this.imageBaseUrl}/c_scale,${dprParameter}${widthParameter}${heightParameter}/${publicId}.${format.toString()}`
    }
}
